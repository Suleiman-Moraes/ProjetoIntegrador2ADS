...
public class TelaCrudCadastrarCurso extends javax.swing.JFrame {

    String nomeDoArquivoCurso = TelaPrincipal.nomeArq+"\\curso.csv";
    CursoDAO banco = new CursoDAO(nomeDoArquivoCurso);
    String butao = "";
    
    public TelaCrudCadastrarCurso() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        jButtonAdicionar.setText("ADICIONAR NOVO CURSO");
        ...
        jButtonVisualizar.setText("VISUALIZAR CURSO");
        ...
        jButtonEditar.setText("EDITAR CURSO");
        ...
        jButtonDeletar.setText("EXCLUIR CURSO");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TelaCrudCadastrarCurso.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TelaCrudCadastrarCurso().setVisible(true);
            }
        });
    }
    private void chamaTelaCadastro(){
        TelaCadastroDeCurso tela = new TelaCadastroDeCurso();
        tela.setLocationRelativeTo(rootPane);
        tela.setVisible(true);
        dispose();
    }
    private void chamaTelaCadastro(Curso curso){
        TelaCadastroDeCurso tela = new TelaCadastroDeCurso(curso);
        tela.setLocationRelativeTo(rootPane);
        tela.setVisible(true);
        dispose();
    }
    ...
    private Curso mostraSoUm() throws Exception{
        try {
            if(jTextFieldCodigo.getText().isEmpty()) throw new Exception("Campo Vazio.");
            if(banco.visualizar(Integer.parseInt(jTextFieldCodigo.getText())) == null)throw new Exception("Não Encontrado.");
            Curso curso = banco.visualizar(Integer.parseInt(jTextFieldCodigo.getText()));
            return curso;
        } catch (Exception e) {
            mensagemErro3();
            throw e;
    ...
    private void saidaUm(Curso curso){
        DefaultTableModel model = (DefaultTableModel) jTablelListagem.getModel();
        model.setNumRows(0);
        
        String[] saida = new String[3];
        saida[0] = curso.getCodigo()+"";
        saida[1] = curso.getDescricao();
        saida[2] = curso.getCoordenado().getNome();
        
        model.addRow(saida);
    }
    private Curso deletar(){
        try {
            jButtonAceitar.setEnabled(false);
            Curso curso = banco.visualizar(Integer.parseInt(jTextFieldCodigo.getText()));
            banco.excluir(curso.getCodigo());
            return curso;
        } catch (Exception e) {
            mensagemErro3();
            JOptionPane.showMessageDialog(rootPane, e.getMessage(), "ERRO", JOptionPane.ERROR_MESSAGE);
    ...
    private ArrayList<Curso> visualizar() throws Exception{
        try {
            jTextFieldCodigo.setEditable(false);
            jButtonAvancar.setEnabled(false);
            ArrayList<Curso> lista = banco.recuperar();
            return lista;
        } catch (Exception e) {
            mensagemErro3();
            throw e;
    ...
    private void saida2(ArrayList<Curso> curso){
        DefaultTableModel model = (DefaultTableModel) jTablelListagem.getModel();
        model.setNumRows(0);
        
        for (int i = 0; i < curso.size(); i++) {
            String[] saida = new String[3];
            saida[0] = curso.get(i).getCodigo()+"";
            saida[1] = curso.get(i).getDescricao();
            saida[2] = curso.get(i).getCoordenado().getNome();
            model.addRow(saida);
        }
    }
